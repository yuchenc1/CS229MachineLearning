...
J = -sum(sum(y_temp.*log(hx)+(1-y_temp).*log(1-hx)))/m+lambda/2/m*sum(theta_temp.^2);
delta_3 = hx - y_temp;
...
Theta1_grad = 1/m*DELTA_1 + lambda/m*[zeros(hidden_layer_size,1) Theta1_temp];
Theta2_grad = 1/m*DELTA_2 + lambda/m*[zeros(num_labels,1) Theta2_temp];
